...
export function IndexTrendDialog({
  ...
  useEffect(() => {
    if (!open || !item) return
    if (byRange[range]) return
    let cancelled = false
    setLoading(true)
    setError('')

    fetchIndexHistory(item.code, range)
      .then((result) => {
        if (cancelled) return
        setByRange((current) => ({...current, [range]: result}))
      })
      .catch((reason: unknown) => {
        const message = (
          reason as {response?: {data?: {message?: string}}; message?: string}
        )?.response?.data?.message
        if (!cancelled) setError(message || (reason as Error)?.message || '加载失败')
      })
      .finally(() => {
