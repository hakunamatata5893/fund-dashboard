...
/**
 * 当日预估仅在 A 股开盘后展示。
 * 法定休市日不维护静态日历，以当天是否实际产生盘中估值点为准。
 */
export function shouldShowIntradayEstimate(
  quote: {
    estimateGrowth?: number | null
    trend?: {growth: number | null}[]
  },
  now = new Date(),
): boolean {
  ...
  if (quote.estimateGrowth == null || !Number.isFinite(quote.estimateGrowth)) {
    return false
  }
  return (quote.trend || []).some(
    (point) => point.growth != null && Number.isFinite(point.growth),
  )
}
